...
const ProductDetails = () => {
  ...
  const product = storedItems.find((product) => product.id === shopId) as ProductProps;
  const { img, category, stock, price, id } = product;
    ...
    <Card sx={{ display: 'flex' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
        <CardContent sx={{ flex: '1 0 auto' }}>
          ...
          {quantity === 0 ? (
            <Button variant='contained' color='secondary' onClick={() => increaseQuantity(id)}>
              add to cart
            </Button>
          ) : (
            <>
              <div>
                <Button variant='contained' color='secondary' onClick={() => decreaseQuantity(id)}>
                  -
                </Button>
              ...
              <div>
                <Button variant='contained' color='primary' onClick={() => removeQuantity(id)}>
                  Remove
                </Button>
              </div>
            </>
